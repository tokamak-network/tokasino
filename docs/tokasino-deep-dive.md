# Tokasino: Randomness-Native OP Stack L2 Deep Dive
...
Tokasino의 목표는 명확하다: **프로토콜 레벨에 검증 가능한 랜덤성을 내장한 L2 체인**을 만들어서, 위의 세 가지 문제를 모두 해결하는 것이다.
...
Tokasino는 독립 체인이 아니라 **OP Stack L2**이다. OP Stack은 Optimism이 만든 L2 프레임워크로, 다음과 같은 인프라를 기본으로 제공한다:
...
이 컴포넌트들은 수정 없이 사용되며, Tokasino의 랜덤성 기능은 전적으로 실행 레이어(EL)에서 처리된다.

#### 계층 2: Tokasino 실행 레이어 (Rust)

표준 OP Stack에서 실행 레이어는 op-geth나 op-reth를 사용한다. Tokasino는 **op-reth를 포크**하여 랜덤성 기능을 추가한다. op-reth는 Rust로 작성된 OP Stack 실행 클라이언트로, 성능과 안정성이 뛰어나다.

Tokasino가 op-reth에 추가하는 것은 세 가지다:
...
op-node가 Engine API를 통해 EL에 블록 생성을 요청할 때, payload attributes에 `prevRandao` 값을 포함한다. Tokasino의 CL은 이 값을 VRF 또는 DRB(Distributed Random Beacon)로 생성한다.
...
2. EL이 페이로드를 빌드한다. 이때 Tokasino의 EL은 전달받은 `prevRandao`를 VRF 출력으로 오버라이드한다.
...
Tokasino에서 VRF 입력은 `parent_hash + block_number`이다. 시퀀서가 자신의 VRF 비밀키로 이 입력을 서명하면, 32바이트의 랜덤 출력과 증명이 생성된다. 이 출력이 해당 블록의 `prevRandao` 값이 된다.

Tokasino는 BLS12-381 곡선 위의 VRF를 사용한다. BLS12-381은 Ethereum 2.0에서도 사용되는 곡선으로, 효율적인 서명 집계(aggregation)가 가능하다는 장점이 있다.
...
Tokasino에서 DRB는 두 가지 방식으로 구현되어 있다:
...
2. **부분 서명**: 매 라운드(블록)마다, 각 노드가 라운드 메시지(`"TOKASINO-ROUND-" + block_number`)를 자신의 비밀키 조각으로 BLS 서명한다.
...
Tokasino는 세 가지 인터페이스를 제공한다. 각각 다른 보안 속성과 사용 사례를 가진다.
...
Ethereum에서 이 값은 밸리데이터가 편향할 수 있지만, Tokasino에서는 VRF로 생성되므로 결정적이고 검증 가능하다.
...
프리컴파일드 컨트랙트(precompiled contract)는 EVM에 하드코딩된 특수 컨트랙트이다. Ethereum에는 ecrecover(0x01), SHA256(0x02) 등이 이미 있다. Tokasino는 주소 0x0b에 랜덤성 프리컴파일을 추가한다.
...
- **TokasinoRandom.sol**: 유틸리티 라이브러리 (random(), rollDice(), coinFlip(), shuffle() 등)
...
SKALE은 현재 프로덕션에서 네이티브 threshold 랜덤을 제공하는 유일한 EVM 체인이다. BLS-RANDAO 방식으로 밸리데이터들이 합의 과정에서 랜덤을 생성한다. 개발자는 `getRandomBytes()`를 호출하면 된다. Tokasino와 가장 유사한 접근이지만, SKALE은 독립 L1이고 Tokasino는 OP Stack L2이다.
...
Move VM에 네이티브 랜덤을 내장했다. `#[randomness]` 어트리뷰트를 사용하면 VM이 자동으로 랜덤을 주입한다. 특히 "test-and-abort" 공격을 VM 레벨에서 방어하는 것이 특징이다 — 트랜잭션이 abort되어도 랜덤은 소비된 것으로 처리한다. Tokasino의 RandomBeaconHistory 접근과는 다른 방식이지만, 같은 문제(revert 공격)를 해결한다.

### Sui

DKG + Threshold 암호화로 합의와 병렬로 랜덤을 생성한다. `sui::random` 모듈을 통해 접근한다. Epoch 기반으로 키를 생성하며, Tokasino의 DRB 모드와 유사한 원리이다.

### Chainlink VRF

오프체인 오라클 방식이다. 어떤 체인에서든 사용할 수 있다는 장점이 있지만, 비용과 지연이 발생한다. Tokasino는 체인 자체에 랜덤을 내장하므로 이 문제가 없다.

| 프로젝트 | 방식 | EVM | L2 | 비용 | 지연 |
|---------|------|-----|----|------|------|
| Tokasino | VRF + DRB precompile | 예 | OP Stack | 무료 | 즉시 |
...
- **CSPRNG (Cryptographically Secure Pseudo-Random Number Generator)**: 암호학적으로 안전한 의사난수 생성기. Tokasino는 ChaCha20을 사용.
- **Precompile**: EVM에 하드코딩된 컨트랙트. 네이티브 코드로 실행되어 효율적.
- **System Contract**: 블록 실행 과정에서 시스템 주소가 자동으로 호출하는 컨트랙트.
- **prevRandao**: EVM 블록 헤더의 필드. 원래 Ethereum에서는 RANDAO 믹스, Tokasino에서는 VRF 출력.
