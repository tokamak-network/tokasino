# Enshrined VRF Randomness: 완전 해설

> 이 문서는 Enshrined VRF 블록체인에서 난수(랜덤)가 어떻게 만들어지고, 블록에 기록되고, 게임에 사용되는지를 처음부터 끝까지 설명합니다. 코드 한 줄 한 줄의 의미를 포함합니다.
...
2. [Enshrined VRF의 해결책: VRF를 합의 레이어에 내장](#2-enshrined-vrf의-해결책-vrf를-합의-레이어에-내장)
...
## 2. Enshrined VRF의 해결책: VRF를 합의 레이어에 내장

Enshrined VRF는 위 문제들을 근본적으로 해결합니다:
...
### Enshrined VRF가 얻는 이점

| 항목 | Chainlink VRF | Enshrined VRF VRF |
|------|-------------|-------------|
...
입력: parent_hash(0xabcd...) || block_number(100)
  ...
  ↓ BLS12-381 서명 (비밀키 + DST "ENSHRINED-VRF-V1")
...
Enshrined VRF는 OP Stack 아키텍처를 따릅니다:
...
- **Enshrined VRF**: **시퀀서의 VRF 출력값**이 들어감
...
Enshrined VRF는 **0x0b**에 난수 생성 프리컴파일을 추가했습니다. 기존 0x0a 다음 번호입니다.
...
Enshrined VRF의 게임 컨트랙트들(InstantDice, CoinFlip, Roulette)이 이 방식을 사용합니다.
...
### 방법 2: Enshrined VRFRandom 라이브러리

게임 컨트랙트에서 더 편리하게 사용할 수 있는 라이브러리도 제공됩니다.

```solidity
// Enshrined VRFRandom.sol
library Enshrined VRFRandom {
...
Alice가 Enshrined VRF에서 주사위 게임을 한다고 합시다.
...
crates/
  ...
  contracts/src/
    IRandomness.sol        — 프리컴파일 인터페이스
    Enshrined VRFRandom.sol     — 난수 유틸리티 라이브러리
